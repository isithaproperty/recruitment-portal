...
export default function Home(){
 ...
 return <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,rgba(200,154,75,.08),transparent_28rem)] bg-[#f4f6f8] text-[#172536]"><div className="mx-auto max-w-7xl px-5 pb-14 pt-5 sm:px-6">
  ...
  {!loading&&interviewRequests.length>0&&<section className="mt-7 rounded-2xl border-2 border-[#c89a4b]/55 bg-[#fff9ec] p-6 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-widest text-[#a88436]">Action required</p><h2 className="mt-1 text-2xl font-extrabold text-[#0b2239]">Interview requests ({interviewRequests.filter(r=>r.interview_status==='requested').length})</h2><p className="mt-1 text-sm text-[#667085]">Clients have selected these candidates for interview. Open Client Submissions to arrange the interview.</p></div><Link href="/client-submissions" className="rounded-lg bg-[#0b2239] px-5 py-3 text-sm font-bold text-white">Manage interviews</Link></div><div className="mt-5 grid gap-3 md:grid-cols-2">{interviewRequests.map(r=><Link key={r.id} href="/client-submissions" className="rounded-xl border bg-white p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-extrabold">{r.candidate_applications?.candidate_name||"Candidate"}</p><p className="mt-1 text-sm text-[#667085]">{r.client_submissions?.recruitment_clients?.company_name||"Client"} · {r.client_submissions?.jobs?.title||"Role"}</p></div><span className="rounded-full bg-[#f3ead8] px-3 py-1 text-xs font-bold uppercase text-[#0b2239]">{r.interview_status.replaceAll('_',' ')}</span></div>{r.interview_scheduled_at&&<p className="mt-2 text-sm font-semibold">{new Date(r.interview_scheduled_at).toLocaleString()}</p>}</Link>)}</div></section>}
