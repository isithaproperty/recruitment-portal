...
export default function JobPage(){
  ...
  return <main className="min-h-screen bg-slate-100"><div className="mx-auto max-w-6xl px-6 py-8">
    ...
    {applyPath&&<div className="mt-6 rounded-lg border border-amber-200 bg-amber-50/50 p-4"><p className="text-sm font-semibold text-slate-900">Candidate application link</p><p className="mt-1 text-xs text-slate-500">Send this public link to candidates. They do not need a recruiter portal login.</p><div className="mt-3 flex flex-wrap gap-2"><code className="min-w-0 flex-1 overflow-x-auto rounded border bg-white px-3 py-2 text-sm">{applyUrl}</code><button className="rounded bg-slate-900 px-4 py-2 text-sm font-semibold text-white" onClick={async()=>{await navigator.clipboard.writeText(applyUrl);setMessage("Application link copied. It is ready to send to candidates.")}}>Copy application link</button><Link className="rounded border border-slate-300 bg-white px-4 py-2 text-sm font-semibold" href={applyPath} target="_blank">Preview form</Link></div></div>}</div>
